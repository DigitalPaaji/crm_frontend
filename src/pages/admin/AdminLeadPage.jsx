...
const AdminLeadPage = () => {
  ...
  const fetchLead = async (id) => {
    ...
    setError(null);
    try {
      const response = await fetch(`${base_url}/leads/admin/get-lead/${id}`, {
        method: "GET",
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${base_url}/leads/admin/update-lead/${leadid}`, {
        method: "PUT",
        headers: {
...
if(data.success){
  ...
  const handleConfirmDelete= async()=>{
    try {
      ...
      const response = await fetch(`${base_url}/leads/admin/delete/${deleteID}`,{
        method:"DELETE",
        headers:{
          Authorization: `Bearer ${token}`
        }
          ...
          <div className="flex gap-3 w-full sm:w-auto">
            ...
            >
             ...
             {/* <button 
              ...
              className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-purple-600 cursor-pointer text-white rounded-lg text-sm font-medium hover:bg-purple-700 transition-colors shadow-sm"
            >
              Follow up
            </button> */}
          </div>     
